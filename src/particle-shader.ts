export const particleVertexShader = /* glsl */ `
  ...
  uniform float uRise;     // rise height in local units
  uniform float uMaxFrac;  // fraction of particles active (0=none, 1=all)

  attribute float aOffset;
  attribute float aSeed;
  attribute float aIndex;  // normalised 0-1 position in the array

  varying float vProgress;
  varying float vSeed;

  void main() {
    // Hide particles beyond the active count threshold
    if (aIndex > uMaxFrac) {
      gl_PointSize = 0.0;
...
export const particleFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // White-yellow at birth → orange-red as they cool
